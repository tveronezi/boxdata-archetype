...
(function () {
    ...
    Ext.define('${rootArtifactId}.view.DeviceUsage', {
        title: ${rootArtifactId}.i18n.get('application.disk.usage'),
        extend: '${rootArtifactId}.ux.Chart',
        alias: 'widget.${rootArtifactId}-device-usage-panel',
        ...
        getSizeString: function (value) {
            ...
            if (number < 1000) {
                return Ext.util.Format.number(number, '0,000.00') + ${rootArtifactId}.i18n.get('megabyte');
            } else {
                return Ext.util.Format.number((number / 1024), '0,000.00') + ${rootArtifactId}.i18n.get('gigabyte');
            }
        },

        getPath: function (path) {
            if (path.length > ${rootArtifactId}.view.DeviceUsage.MAX_PATH_LENGTH) {
                var arr = path.split('\\');
                if (arr.length === 1) {
                    arr = path.split('/');
                }
                if (arr.length === 1) {
                    return Ext.String.ellipsis(path, ${rootArtifactId}.view.DeviceUsage.MAX_PATH_LENGTH);
                } else {
